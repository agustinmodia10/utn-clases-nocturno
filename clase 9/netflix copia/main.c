...
void cargarClientes(eCliente clientes[])
{
    ...
    int idSerie[10]={100,100,101,103,103,101,102,103,100,104};
        int idSerie2da[10]={104,101,100,102,102,103,100,104,101,100};
            int idSerie3era[10]={100,100,101,103,103,104,102,103,103,100};
    int i;
    ...
    for(i=0; i<10;i++)
    {
        ...
        clientes[i].idSerie2da = idSerie2da[i];
        clientes[i].idSerie3era = idSerie3era[i];
...
void mostrarClientes (eCliente cliente[], int num)
{
    ...
    for(e = 0; e < num; e++)
	{
        ...
        {
            printf("\n  nombre: %s\n Id cliente: %d \n Id serie: %d \n Id 2da serie: %d \n Id 3era serie: %d \n", cliente[e].nombre, cliente[e].idCliente, cliente[e].idSerie, cliente[e].idSerie2da, cliente[e].idSerie3era);
        }
...
void mostrarClientesConSerie(eCliente cliente [], eSerie serie [], int num, int nums)
{
     ...
     for(e = 0; e < num ; e++)
    {
        ...
        for(l = 0 ; l < nums; l++)
        {
         ...
         for(l = 0 ; l < nums; l++)
        {
            if(serie[l].idSerie==cliente[e].idSerie2da)
			{
				printf(" serie: %s \n", serie[l].titulo );
         ...
         for(l = 0 ; l < nums; l++)
        {
            if(serie[l].idSerie==cliente[e].idSerie3era)
			{
				printf(" serie: %s \n", serie[l].titulo );
     ...
     for(e = 0; e < nums ; e++)
    {
        ...
        for(l = 0 ; l < num; l++)
        {
            ...
            if(serie[e].idSerie==cliente[l].idSerie)
			{
			 ...
			 if(serie[e].idSerie==cliente[l].idSerie2da)
			{
			    printf("  -> cliente: %s \n", cliente[l].nombre );

			}
			 if(serie[e].idSerie==cliente[l].idSerie3era)
			{
			    printf("  -> cliente: %s \n", cliente[l].nombre );
     ...
     for(e = 0; e < nums ; e++)
    {
        ...
        for(l = 0 ; l < num; l++)
        {
            ...
            if(serie[e].idSerie==cliente[l].idSerie && cliente[l].idSerie == 100)
			{
			 ...
			 if(serie[e].idSerie==cliente[l].idSerie2da && cliente[l].idSerie2da == 100)
			{
			    printf("  -> cliente: %s \n", cliente[l].nombre );
                    cont++;
			}
			if(serie[e].idSerie==cliente[l].idSerie3era && cliente[l].idSerie3era == 100)
			{
			    printf("  -> cliente: %s \n", cliente[l].nombre );
                      cont++;
			}
         ...
         printf("\nla cantidad de personas que lo ven son: %d\n",cont);
   ...
   for(e=0; e<ser; e++)
          ...
          for(e = 0; e < ser ; e++)
    {
        ...
        for(l = 0 ; l < cli; l++)
        {
              ...
              if( cliente[l].idSerie2da == loser[e].idSerie )
              {
                  loser[e].cont++;
              }
              if( cliente[l].idSerie3era == loser[e].idSerie )
              {
                  loser[e].cont++;
              }
